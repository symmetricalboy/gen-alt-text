...
By consistently applying these guidelines, you will create alt-text for video thumbnails that is informative, concise, and helpful for users of assistive technology.`;async function E(e,n){if(e.startsWith("data:")){console.log("[getBase64Data] Source is Data URL, extracting...");const o=e.match(/^data:(.+?);base64,(.*)$/);if(!o||o.length<3)throw console.error("[getBase64Data] Invalid Data URL format received:",e.substring(0,100)+"..."),new Error("Invalid Data URL format received from content script");const a=o[1],i=o[2];return console.log("[getBase64Data] Extracted mimeType:",a,"data length:",i.length),{base64Data:i,mimeType:a}}else throw console.error("[getBase64Data] ERROR: Received non-Data URL source despite changes:",e.substring(0,100)+"..."),new Error("Background script received a non-Data URL source unexpectedly.")}async function b(e,n,o){var a,i,c,r,v;try{const{base64Data:s,mimeType:g}=await E(e,o);console.log(`Generating alt text for ${n?"video":"image"} (type: ${g})`);const A="https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent",P={contents:[{parts:[{text:n?k:y},{inline_data:{mime_type:g,data:s}}]}]},d=await fetch(`${A}?key=${t}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(P)});if(!d.ok){const x=await d.text();throw console.error("Gemini API error response:",x),new Error(`Gemini API error: ${d.status} - ${x.substring(0,100)}...`)}const w=await d.json(),u=(v=(r=(c=(i=(a=w.candidates)==null?void 0:a[0])==null?void 0:i.content)==null?void 0:c.parts)==null?void 0:r[0])==null?void 0:v.text;if(console.log("Extracted alt text:",u),!u)throw console.error("Generated text missing from Gemini response:",w),new Error("Could not extract text from Gemini response");return{altText:u.trim()}}catch(s){return console.error("Error in generateAltTextForMedia:",s),{error:s instanceof Error?s.message:"Unknown error during generation"}}}m.runtime.onConnect.addListener(e=>{console.log(`Port connected: ${e.name}`,e.sender),e.name==="altTextGenerator"&&(e.onMessage.addListener(async n=>{if(console.log("Port message received:",n),n.action==="generateAltText"&&n.imageUrl&&typeof n.isVideo=="boolean"){console.log(`Port request: Generate alt text for ${n.imageUrl}, isVideo: ${n.isVideo}`);const o=await b(n.imageUrl,n.isVideo,void 0);console.log("Sending result back via port:",o);try{e.postMessage(o)}catch(a){console.error("Error posting message back via port:",a)}}else console.warn("Received unknown message format or action via port:",n)}),e.onDisconnect.addListener(()=>{console.log(`Port ${e.name} disconnected.`)}))}),m.runtime.onMessage.addListener(async(e,n,o)=>{var a;if(console.log("General message received:",e.type,"from sender:",(a=n.tab)==null?void 0:a.id),e.type==="MEDIA_INTERCEPTED"&&e.payload){const i=e.payload;console.log(`Intercepted media: ${i.filename} (${i.filetype}, ${i.size} bytes)`),console.log("Attempting immediate generation for intercepted media...");const c=i.filetype.startsWith("video/"),r=await b(i.dataUrl,c,i.filetype);return console.log("Result for intercepted media:",r),o({status:"Intercepted and processed",filename:i.filename,result:r}),!0}else console.log("Ignoring unknown message type or missing payload:",e.type);return!1}),console.log("Background script event listeners attached.")});function $(){}function l(t,...y){}const D={debug:(...t)=>l(console.debug,...t),log:(...t)=>l(console.log,...t),warn:(...t)=>l(console.warn,...t),error:(...t)=>l(console.error,...t)};let h;try{h=T.main(),h instanceof Promise&&console.warn("The background's main() function return a promise, but it must be synchronous")}catch(t){throw D.error("The background crashed on startup!"),t}return h}();
background;
